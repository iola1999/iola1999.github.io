...
本文基于个人设备、个人账号、官方已安装的 macOS 钉钉客户端。目的是学 ObjC Runtime、DYLD 注入和客户端逆向，不涉及破解软件分发、绕过账号鉴权，也没有攻击服务端或动别人数据。

逆向、调试、Hook 都只该在你自己拥有、收发双方都是你自己的设备上做。拦掉已读回执，改的是你自己这台机器上客户端的行为，别拿去给组织或上级隐瞒本该知悉的工作信息，也别用来规避正常的工作协同。
