...
战绩接口一次只返回最近十局，所以这个问题只能查到一部分人。五人人机队对面共有 2,383 条真人参战记录，涉及 2,012 名玩家，其中 625 条能在库里找到上一局。按这 625 条记录统计，上一局输了 519 条，败率 83.04%。

同一个人可能多次碰到五人人机。为了做普通排位对照，我先给每名玩家只留最早一次遭遇，2,012 人中还剩 254 人能回溯上一局，覆盖率 12.62%。这 254 人上一局 33 胜、221 负，胜率 12.99%。这个去重口径只用于分层对照，后面的多局窗口会把每次遭遇都算进去。

低分段玩家遇到人机的频率本来就高，对照也要管住段位和日期。我按北京时间自然月和当局累计星数 5 星一档，把这些玩家与同期普通排位匹配，同时排除所有进入过五人人机组的玩家。254 人中有 249 人所在分层能找到对照：

| 上一局表现 | 五人人机局对手 | 普通排位对照 |
| --- | ---: | ---: |
| 胜率 | **11.65%** | 65.53% |
| 平均评分 | **7.538** | 8.465 |
...
![完整五人人机队的胜负，以及对手上一局与同期同分段普通排位的胜负和表现对照](/upload/images/2026-07-28-WZRY-Analysis-2/ai-opponent-previous.png)

分月、分段以后，上一局胜率仍差 53.88 个百分点。目标组的死亡更多，助攻和 KDA 也低很多，击杀数则很接近。数据库里能查到的上一局表现，与随后碰到完整人机队有很强的关联。这个对照只覆盖 249 名玩家，触发条件仍然未知；历史战绩缺失的玩家也可能改变总体结果。

上一局只能看到最后一个点。我又以每次遭遇为单位，回溯最近 1、3、5、8、10 条已采集排位记录。为了让五个窗口可以直接比较，下面只用能回溯满 10 条的固定样本：402 条遭遇记录，涉及 167 名玩家和 372 场五人人机局，覆盖全部 2,383 条遭遇记录的 16.87%。
...
![每次碰到完整五人人机队前，最近十条已采集排位记录的逐局胜率和不同回溯窗口分布](/upload/images/2026-07-28-WZRY-Analysis-2/ai-opponent-history.png)
...
![两种人机安排按北京时间自然日统计的每日场次数，以及占当天全部完整排位的比例](/upload/images/2026-07-28-WZRY-Analysis-2/ai-daily-distribution.png)
...
「牢玩家」是玩家自嘲用语，借了「老玩家」的同音，社区里没有统一口径。常见说法还会带上评分偏高、胜率长期在五成附近，以及总被分到难打对局的主观体验。数据库只能看到账号年限和累计场次，下文只借这个称呼描述这两项档案字段。
...
最强的几组大多来自口径重叠：累计星数与 MMR 是 0.997，MMR 与 ELO 是 0.994，皮肤收藏值与总收藏值是 0.992。对战场次与被赞为 0.902，与排位场次为 0.901，与王者次数为 0.834。新增的累计 MVP 也一样，它与总场次的相关系数是 0.862，与累计被赞是 0.792。这些累计量都会随游戏总量增长，正文里逐项解释没有多少价值。
