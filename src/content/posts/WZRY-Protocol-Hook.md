...
>
> 2026-07-27 更新：补一节前端工作台，规则从设备上的配置文件搬到了主机的服务里。

这篇是一天的流水账，从 tcpdump 一路挖到 IL2CPP 的协议层。网络层试了三种改法，Frida 试了两种进法，成熟的 hook 框架在 217MB 的库上装不进去，最后落到手写 inline hook 加运行时 dump 符号表。现在能按字段名改任意一条收发消息，长度不受原值限制，HTTP 的 URL 也能记下来。规则后来搬到了主机上的一个工作台里，最后一节讲这个。
...
6 个字的限制在客户端的改名界面上，schema 的 64 字节在协议定义里，这两道都没拦住，因为改的是 `unpack` 之后、进游戏逻辑之前的那个对象。这条链路走通之前还踩了个 8 字节的坑，放到后面讲。先说走到这一步之前的过程。
...
处置有四种：`allow` 原样放行、`patch` 按字段路径改、`drop` 在发包侧丢掉、`respond` 不真发直接给一份响应。前三种通了，`respond` 要用到游戏自带的 `CreateDefaultCSPKG` 和 `LobbyMsgRecvSimulated`，这轮只把它们挂上去看行为，还没真调。顺带说一下 `CreateDefaultCSPKG` 挂不上：dump 给的 RVA 是 243MB，超过了 `libil2cpp.so` 本身的 218MB，是个无效地址，装 hook 前读 prologue 那一下裸解引用当场把进程带走了，现在改成先 `safe_read` 探一下，探不通就跳过。

请求响应配对也顺手弄清楚了。本来指望 `CSPkgHead` 里的 `dwSvrPkgSeq` 能当请求 id，实测下来它是服务器给自己的包编号，客户端在出包里回显最近收到的那个，服务器主动推的通知这个字段是 0，属于 ack 性质，配不了对。最后改成按 cmdid 的命名规律配，`CSID_xxx_REQ` 对 `SCID_xxx_RSP`，八成的请求能配上响应，前端就能显示往返耗时。

这一节的东西还在做，服务端和前端刚跑起来，写这段的时候库里有一千九百多条消息。

## 改一下金币和钻石

工作台跑起来之后，第一件事是把规则从设备上的配置文件挪到前端，先拿昵称那条验证远端下发这条链路。中间踩了两个坑。`synctimeout` 我按 RTT 实测的 P99 设成了 5ms，可 ping 量的是 30 字节空包，真实事件的 JSON 有 5KB，同步决策直接超时，设备压根没收到回话；放宽到 50ms 之后，日志里是 `决策回话 8323us`。另一个是 JSON 字符串值解出来本身带引号，我又给包了一层，昵称显示成了带引号的 `"远端下发的名字"`。

然后就轮到货币。第一反应是在实时流里全文搜界面上显示的金币数，一条都没命中，因为金币在登录响应里，而那次会话没重新登录过。换个方向，从 schema 里搜名字像货币的字段，`SCPKG_CMD_GAMELOGINRSP` 里有个 `stCoinList`，往下是 `CoinCnt`，一个 `UInt32[]`，按货币类型索引。
...
得说清楚的是，这纯粹是客户端显示层的事。改的是 `unpack` 之后那个对象，服务器自己那本账一分没动，拿这些钱去买东西，请求发过去照样被打回来。它证明的是链路本身：前端点几下下发的规则，能精确落到某条响应的某个数组元素上。点券不在 `CoinCnt` 里，它有自己的消息 `SCPKG_CMD_ACNTCOUPONS`（1161/1162），字段是 `llCouponsCnt`，还没验。

再往前一步倒是有点意思。消息历史里现在躺着几千条真实往返，而 `respond` 这个处置是不真发、直接给一份响应。录和放都有了，剩下的是让服务端自己算，照着真实服务器的收发把逻辑实现出来，客户端连过来就是一个私服。区服列表那条链路早就 hook 住了，往响应里塞一个自己的服务器条目也是通的。这条我记在计划里，本轮不做。

## 尾声

剩下的坑：所有 RVA 绑死在 11.4.1.1，游戏一更新全部作废，得重新 dump 再定位一遍，这一步现在纯手工。C# 那条 HTTP 通道（`UnityWebRequest.SendWebRequest`）还没做，原生那条已经覆盖了 MSDK 和 FaaS，暂时够用。全量 dump 只能在大厅跑，登录重试期间触发过一次 SIGABRT，栈顶是 il2cpp 给某个类型拼名字时自己断言了，现在会跳过泛型参数、指针这几类拿不到名字的类型，但规矩没改。工作台那边还有一半没做完，`respond` 处置、curl 的请求体和响应体、内联 ValueType 的中间段下钻都排在后面。

回头看这两天，费时间的地方不在写代码。每次一条路走不通，都得先判断到底是方案本身不行、还是我用错了。前者要掉头，后者只要接着修。判断错了就是几个小时白花。等工作台那套东西成型、版本绑定这件事有个像样的办法，再写下一篇。
