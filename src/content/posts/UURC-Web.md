...
很久没写这种长篇考古记录了。最近又开了个坑：[UURC-Web](https://github.com/iola1999/uurc-web)。
...
界面上的改动比较杂。画面源会标出分辨率和「无信号」，切到空视频轨时直接显示「该画面暂无内容」，黑屏的原因一眼就能看到；命令栏平时停在画面上方，进入全屏后才变成可拖动浮层；「仅查看 / 控制中」可以随时暂停输入，连接和画面状态也换成了能看懂的中文。

声音走独立播放器，工具栏可以静音和调音量，浏览器拦住自动播放时按钮会提示手动恢复。
...
剪贴板两个方向都通。本机粘贴通过文本通道发到远端，受控端回传的 `RomMsg_Text` 写进浏览器剪贴板，浏览器拒绝写入时内容会留在面板里，可以手动取用。
...
这个项目里最花时间的是把 native 层的调用链、协议字段和网页端实现对起来，`libstreamer.so` 那几万行伪代码靠我自己读会非常慢。AI 做这部分效率比我高，我负责把现象说清楚、判断它查得有没有跑偏，再拿设备验证。这个分工在这个项目上是成立的。

项目代码已经开源，有类似需求、或者想看看这套远控协议怎么跑，可以直接去仓库翻代码。
